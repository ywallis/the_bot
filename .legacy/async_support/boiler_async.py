...
def order_book_matcher(
    ...
    while dynamic_arb:
        ...
        if highest_bid_price >= lowest_ask_price * spread:
            ...
            print(
                f"Optimal spread currently between {lowest_ask_price} and {highest_bid_price}."
            ...
            if order_size < min_order_size:
                ...
                logger.info(
                    f"Below minimum order size, increasing to {min_order_size}."
                )
...
async def place_sell_order(
    pair: str, client: Exchange, price: float, quantity: float, identifier: str
) -> Order:
    ...
    order: Order = await client.create_limit_order(
        symbol=pair,
        side="sell",
        amount=quantity,
        price=price,
...
async def place_buy_order(
    ...
    if client.name == "Gate.io":
        ...
        print(
            f"Placing a {quantity_with_fee} {pair} buy order on {client.name} for {price}."
        )
        logger.info(
            f"Placing a {quantity_with_fee} {pair} buy order on {client.name} for {price}."
        )
        ...
        logger.info(
            f"Placing a {quantity} {pair} buy order on {client.name} for {price}."
        )
...
async def fetch_balances(
    buy_client: Exchange, sell_client: Exchange
...
async def check_and_take(
    client_a: Exchange, client_b: Exchange, order: Order, pair: str
) -> bool:
    ...
    if filled != 0.0:
        if func_order["status"] == "open":
            ...
            try:
                ...
                filled_str = await client_b.fetch_order(
                    id=func_order["id"], symbol=pair
                )
        ...
        try:
            await client_a.create_market_order(
                symbol=pair,
                side=market_side,
                amount=filled,
                price=order["price"],
...
async def create_and_return_order_abstraction(
    maker_client: Exchange, price: float, size: float, pair: str, side: OrderSide
) -> Order:
    ...
    try:
        order: Order = await maker_client.create_limit_order(
            symbol=pair,
            side=side,
            amount=size,
            price=price,
...
async def cancel_order_abstraction(maker_client: Exchange, order: Order, pair: str):
    ...
    logger.error(
        "Order could not be cancelled after multiple retries. Logging order details"
    )
    logger.error(order)
...
async def take_take(
    ...
    sell_client: Exchange,
    pair: str,
    sell_client_bids: list[list],
    buy_client_asks: list[list],
    spread: float,
    sizing: float,
    max_order_size: float,
    spread_extension: int,
) -> bool:
    try:
        taker_target_ask, taker_target_bid, taker_order_size = order_book_matcher(
            sell_client_bids,
            buy_client_asks,
            spread,
            sizing,
        ...
        if await check_if_solvent(
            buy_client,
            sell_client,
            quantity=taker_order_size,
            price=taker_target_ask,
            pair=pair,
        ):
            try:
