...
- Replayed processes coordinate through keys under the prefix (`streams.replay_progress_key`, `streams.replay_done_key`): a replayed `Runtime` writes its clock after every read and stops when the done key exists and its streams are drained; the replayer's `--follow <name>` keeps the published data within `--lookahead` seconds of the slowest consumer. Read these keys through `streams.read_progress`; do not invent a second coordination channel.
