...
- `apps/maker/src/replayer.py` is the recorder run backwards: `uv run -m apps.maker.src.replayer <run_id> [--start ISO --end ISO] [--speed N]` publishes a recording under `bt:<run_id>:` with the original entry ids and payloads, streams merged on `ts_recv` but never reordered within a stream, the last book and balance before the range replayed first. It refuses an empty prefix. A strategy is pointed at the replay with `uv run -m apps.maker.src.launcher <index> --replay <run_id>`, which is `Runtime(..., prefix=backtest_prefix(run_id), clock=Clock.replay())`: under a replay clock the runtime reads every stream from its start and does not prime.
- Replayed processes coordinate through keys under the prefix (`streams.replay_progress_key`, `replay_frontier_key`, `replay_done_key`, `replay_broker_key`, `replay_closed_key`): a replayed `Runtime` writes its clock after every read, or the frontier when the read found nothing, and stops when the done key exists and its streams are drained, or when the closed key exists if a simulated broker announced itself; the replayer's `--follow <name>` keeps the published data within `--lookahead` seconds of the slowest consumer. Read these keys through `streams.read_progress` and `streams.read_frontier`; do not invent a second coordination channel.
- `apps/maker/src/sim_broker.py` is the venue side of a backtest: `uv run -m apps.maker.src.sim_broker <run_id> --follow <strategy> [--assume-rtt-ms VENUE=MS] [--report out.json]`. It reproduces the order manager's slot, lock and coalescing semantics, draws latency from `apps/maker/src/latency.py` (measured `oms:latency` records; a venue without any is refused unless assumed explicitly), fills against the recorded book of the arrival time and a queue-position model, charges `[backtest.fees]` in the asset received, and publishes order events, latency records and balances under the prefix. The matcher runs under a backtest with `uv run -m apps.maker.src.matcher --replay <run_id>`. The order to start things and the knobs are in `docs/runbooks/backtest.md`.
...
- The runtime delivers a multi-stream `XREAD` batch merged on `ts_recv`, never reordered within a stream, and holds back entries received after the last entry of any stream that filled its batch. Do not bypass `Runtime.step` to hand a strategy raw `XREAD` blocks; stream by stream is not the order the market moved in.
