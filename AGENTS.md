...
│   │   ├── ccxt_orders.py  # has-map-driven order listing over REST, shared by maker and accountant
│   │   ├── fees.py         # Fee schedule model, fee-currency resolution and CCXT loaders
...
- `apps/maker/src/fees.py` fetches each venue's fee schedule at startup and every `fees.refresh_s`, publishing `FeeScheduleEvent`s to `acct:fees:<venue>` and the `fees-<venue>` snapshot key. Rates come from `fetch_trading_fees` where the venue has it, market defaults otherwise, and config overrides last; the fee currency policy lives on the venue in config and is verified against what fills report.
...
- Before changing a strategy's spread, ask whether the market can pay it: `uv run -m apps.maker.src.tools.market_screener --minutes 10` ranks every spot market the configured venues share by touch offset, taker depth and the print volume landing past the taker touch at the fee budget. Its figures are REST samples and upper bounds; record the shortlist and re-measure on every book update before trading it. For every symbol whose asset has a linear perpetual on a configured derivatives venue (or, until one is declared, on the spot venues' own listings) it also prices the inventory hedge: basis of the perpetual bid over the spot bid, current and trailing-week funding from `apps/shared/src/funding.py`, and the round trip net of both legs' fees; `--perp unified` keeps only symbols whose perpetual sits on a unified account, which is the screening criterion for the next inventory candidate.
- `uv run -m apps.maker.src.tools.pnl [--root data] [--start ISO] [--end ISO]` values every hedged maker fill in a recording in quote units: maker fill against the hedge sharing its client order id, fees from the fill, the command line, the venue's static rates or the recorded schedule, residual base marked at the hedge price. Run it after every live session; the accountant's balances do not say whether a fill made money.
- A venue is an account, not an exchange. `[[venues]]` entries carry `market_type` (`spot`, the default, or `swap` for a linear perpetual wallet), `account` (`classic` or `unified`), `ccxt_id` when the CCXT class differs from the venue id (the same exchange's futures wallet is a second entry sharing its class), `credentials` when two entries share one key, and `margin_mode`, `leverage` and `leverage_params` (one leverage call per parameter set, for venues that keep leverage per margin type and side) for derivatives. A contract symbol is `BASE/QUOTE:SETTLE` in CCXT's form; `config.is_contract` tells the two kinds apart, `fees.base_quote` strips the settle suffix, and a symbol subscribed on a venue whose market type cannot trade it is a config error. `SymbolFees.contract_size` is set on contract markets, where an order amount is in contracts, not base. `OrderIntent.reduce_only` travels to CCXT's `reduceOnly`; the order manager rejects it on a spot symbol. `SELF_HEDGED_TAG` marks an intent whose hedge the strategy places itself. The broker applies every derivatives venue's margin mode, leverage and one-way position mode at start (`apps/shared/src/derivatives.py`) and exits if a venue refuses. Run `venue_conformance <venue> <BASE/QUOTE:QUOTE> --market-type swap --place-orders` before a futures account trades. Design: `docs/design/inventory-hedging.md`.
- Maker quotes are post-only by default (`post_only = false` under a strategy's params turns it off): an `OrderIntent` with `time_in_force = post_only` becomes `post_only` on the broker message and CCXT's `postOnly` param, and a venue refusal comes back as a rejection that frees the slot. The order watcher reconciles over REST every minute on a healthy socket as well as after every drop (`RECONCILE_EVERY_S`). The balance feed overlays every `watch_balance` update onto the last full snapshot before publishing, because some venues deliver deltas and a `BalanceEvent` is a full snapshot by contract. On shutdown the order manager marks itself closing and rejects intents before it sweeps the book, so nothing is placed behind the sweep.
