...
async def cancel_order_return_confirmation(cancellation: CancellationMessage, client: CustomExchange):
    ...
    for attempt in range(5):
        ...
        try:
            ...
            logger.error(f"Cancellation timed out, attempt num.:{attempt + 1}. Error was: {e}")
            last_error = e
            await asyncio.sleep(0.2)

        except ExchangeError as e:

            logger.error(f"Cancellation rejected by exchange, attempt num.:{attempt + 1}. Error was: {e}")
            last_error = e
            await asyncio.sleep(0.2)
    ...
    raise BrokerError(f"Broker was unable to confirm a cancellation. Last error was {last_error}") 
...
async def create_and_return_order(order: OrderMessage, client: CustomExchange):
    ...
    raise BrokerError(f"Broker was unable to confirm an order. Last error was {last_error}") 
